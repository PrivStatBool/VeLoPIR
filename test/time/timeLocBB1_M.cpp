...
int main() {
    ...
    int lengthInterval = 16;  // Length for interval values (x, y, x_left, x_right, y_left, y_right)
    int lengthService = 16;   // Length for service values
    ...
    std::ofstream file("result/generalTimeLocBB1.csv");
    file << "M,NONE,PARALLEL_LOOP_HOMSUM,PARALLEL_LOOP_HOMSUM_BB1_BITWISE,ALL\n";
    ...
    std::vector<int> M_sizes = {8, 16, 24, 32, 40, 48, 56, 64};
    ...
    for (int M : M_sizes) {
        ...
        for (auto mode : modes) {
            ...
            std::cout << "Finished M=" << M << ", mode=" << static_cast<int>(mode) << ", time=" << elapsed.count() << "s" << std::endl;
        }
    ...
    std::cout << "Test completed and results saved to result/generalTimeLocBB1_M.csv" << std::endl;
