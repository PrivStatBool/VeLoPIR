...
double testHomCompL(LweSample* a, LweSample* b, int length, const TFheGateBootstrappingCloudKeySet* bk, int np) {
    ...
    if (np == 0) {
        HomCompL(result, a, b, length, bk);  // Non-optimized version
    } else {
        HomCompLOPT(result, a, b, length, bk, np);  // Optimized version with np threads
    }
...
double testHomCompLGPU(LweSample* a, LweSample* b, int length, const TFheGateBootstrappingCloudKeySet* bk, int num_of_cores) {
    LweSample* result = new_gate_bootstrapping_ciphertext_array(1, bk->params);
    ...
    if (num_of_cores == 0) {
        ...
        HomCompLGPU(result, a, b, length, bk, num_of_cores);  // Optimized version with np threads
    }
...
int main() {
    ...
    std::ofstream file("result/HomCompL_128.csv");
    ...
    for (int np : np_values) {
        ...
        for (int length : lengths) {
            ...
            double elapsed_cpu = testHomCompL(a, b, length, bk, np);
            file << "," << elapsed_cpu;
        ...
        for (int length : lengths) {
            ...
            double elapsed_gpu = testHomCompLGPU(a, b, length, bk, np);
            file << "," << elapsed_gpu;
    ...
    std::cout << "Test completed and results saved to result/HomCompL_128.csv" << std::endl;
    return 0;
}
