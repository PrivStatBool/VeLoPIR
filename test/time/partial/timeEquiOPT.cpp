...
// Function to perform the test and return the elapsed time (CPU)
double testHomEqui(LweSample* a, LweSample* b, int length, const TFheGateBootstrappingCloudKeySet* bk, int np) {
    ...
    if (np == 0) {
        HomEqui(result, a, b, length, bk);  // Non-optimized version
    } else {
        HomEquiOPT(result, a, b, length, bk, np);  // Optimized version with np threads
    }
...
double testHomEquiGPU(LweSample* a, LweSample* b, int length, const TFheGateBootstrappingCloudKeySet* bk, int num_of_cores) {
    LweSample* result = new_gate_bootstrapping_ciphertext_array(1, bk->params);
    ...
    if (num_of_cores == 0) {
        ...
        HomEquiGPU(result, a, b, length, bk, num_of_cores);  // Optimized version with np threads
    }
...
int main() {
    ...
    std::ofstream file("result/HomEqui_128.csv");
    ...
    for (int np : np_values) {
        ...
        for (int length : lengths) {
            ...
            double elapsed_cpu = testHomEqui(a, b, length, bk, np);
            file << "," << elapsed_cpu;
        ...
        for (int length : lengths) {
            ...
            double elapsed_gpu = testHomEquiGPU(a, b, length, bk, np);
            file << "," << elapsed_gpu;
    ...
    std::cout << "Test completed and results saved to result/HomEqui_128.csv" << std::endl;
    return 0;
}
