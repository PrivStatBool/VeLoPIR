...
void test_BB1OPT(const int length, const TFheGateBootstrappingCloudKeySet* bk, const TFheGateBootstrappingSecretKeySet* key, int num_of_threads) {
    ...
    BB1OPT(res, x1, y1, loc, length, bk, num_of_threads);
    assert(!isDecryptedResultOne(res, key));  // Should be 0
    std::cout << "Test 1 (BB1OPT) passed." << std::endl;
    ...
    BB1OPT(res, x2, y2, loc, length, bk, num_of_threads);
    assert(!isDecryptedResultOne(res, key));  // Should be 0
    std::cout << "Test 2 (BB1OPT) passed." << std::endl;
    ...
    BB1OPT(res, x3, y3, loc, length, bk, num_of_threads);
    assert(!isDecryptedResultOne(res, key));  // Should be 0
    std::cout << "Test 3 (BB1OPT) passed." << std::endl;
    ...
    BB1OPT(res, x4, y4, loc, length, bk, num_of_threads);
    assert(isDecryptedResultOne(res, key));  // Should be 1
    std::cout << "Test 4 (BB1OPT) passed." << std::endl;
    ...
    std::cout << "BB1OPT function passed all tests." << std::endl;
}

void test_BB1OptGPU(const int length, const TFheGateBootstrappingCloudKeySet* bk, const TFheGateBootstrappingSecretKeySet* key, int num_of_threads) {
    // Define the bounding box (x_left, x_right, y_left, y_right)
    double x_left = 1.0;
    double x_right = 3.0;
    double y_left = 2.0;
    ...
    LweSample* x1 = encryptBoolean(encodeDouble(length, 0.5), length, bk->params, key);
    LweSample* y1 = encryptBoolean(encodeDouble(length, 1.5), length, bk->params, key);
    BB1OptGPU(res, x1, y1, loc, length, bk, num_of_threads);
    assert(!isDecryptedResultOne(res, key));  // Should be 0
    std::cout << "Test 1 (BB1OptGPU) passed." << std::endl;

    // Test 2: x is out of bounding box while y is in
    LweSample* x2 = encryptBoolean(encodeDouble(length, 0.5), length, bk->params, key);
    LweSample* y2 = encryptBoolean(encodeDouble(length, 2.5), length, bk->params, key);
    BB1OptGPU(res, x2, y2, loc, length, bk, num_of_threads);
    assert(!isDecryptedResultOne(res, key));  // Should be 0
    std::cout << "Test 2 (BB1OptGPU) passed." << std::endl;

    // Test 3: y is out of bounding box while x is in
    LweSample* x3 = encryptBoolean(encodeDouble(length, 2.5), length, bk->params, key);
    LweSample* y3 = encryptBoolean(encodeDouble(length, 1.5), length, bk->params, key);
    BB1OptGPU(res, x3, y3, loc, length, bk, num_of_threads);
    assert(!isDecryptedResultOne(res, key));  // Should be 0
    std::cout << "Test 3 (BB1OptGPU) passed." << std::endl;

    // Test 4: (x, y) is inside the bounding box
    LweSample* x4 = encryptBoolean(encodeDouble(length, 2.5), length, bk->params, key);
    LweSample* y4 = encryptBoolean(encodeDouble(length, 3.5), length, bk->params, key);
    BB1OptGPU(res, x4, y4, loc, length, bk, num_of_threads);
    assert(isDecryptedResultOne(res, key));  // Should be 1
    std::cout << "Test 4 (BB1OptGPU) passed." << std::endl;
    ...
    std::cout << "BB1OptGPU function passed all tests." << std::endl;
}
...
int main() {
    ...
    int num_of_threads = 32;  // Set the number of threads for the test

    // Run tests for BB1OPT
    test_BB1OPT(length, bk, key, num_of_threads);

    // Run tests for BB1OptGPU
    test_BB1OptGPU(length, bk, key, num_of_threads);

    // Clean up keys
    delete_gate_bootstrapping_secret_keyset(key);
    delete_gate_bootstrapping_parameters(params);

    std::cout << "All BB1OPT and BB1OptGPU function tests passed." << std::endl;
    return 0;
}
